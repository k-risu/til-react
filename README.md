...
- 모든 컴포넌트는 웹브라우저 새로고침하기전까지 `state 를 유지` 합니다.

### 4.1. 기준을 세워드릴게요

- 리액트 컴포넌트에서 사용하는 변수는 그냥 `useState()`로 만드세요.
- 컴포넌트를 변수를 변경해서 리랜더링이 필요한 경우 `useState()`
...
const Sample0 = () => {
  ...
  // 클릭하면 set 으로 리랜더링 하겠다.
  // 연속으로 업데이트 안됨(비동기라서 함수완료 후 반영)
  const click = () => {
    // setcount(count + 1);
    // setcount(count + 1);
...
- 리랜더링에서 제외됨.
- 백앤드 API 호출시 좋은 자리 : 자료를 만들고 화면을 보여주는 흐름
- 컴포넌트 최초 데이터 셋팅 : state 값에 기본값 넣기

### 6.2. 최초 컴포넌트 보일때 용도

- 최초 화면에 컴포넌트 보이면 딱! 한번 실행(함수, setState 한번만...)
- 최초 화면에 컴포넌트 보일떄 필요로 한 정보를 `백엔드 데이터 가지고 올때` 딱! 한번 실행
...
### 6.3. 컴포넌트의 state 가 변하는 것을 체크하고자 할때
...
useEffect(() => {
  ...
  return () => {
    window.remoneEventListner("resize", () => console.log("해해"));
